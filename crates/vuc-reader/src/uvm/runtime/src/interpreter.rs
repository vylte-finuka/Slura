...
fn is_valid_jumpdest(
    ...
    if dest < prog.len() {
        ...
        // Log pour debug
        println!("⚡ [JUMP PATCH] Saut autorisé vers 0x{:04x} (opcode=0x{:02x})", dest, prog[dest]);
        return true;
    } else {
...
0x1b => {
    if evm_stack.len() < 2 { return Err(
        Error::new(ErrorKind::Other, "EVM STACK underflow on SHL")
    ); }
...
0x35 => { // CALLDATALOAD
    ...
    if offset <= 4 {
        ...
        println!("🛡️ [CALLDATALOAD PATCH] Selector forcé pur (offset={})", offset);
    }
    ...
    if debug_evm {
        ...
        println!("📥 [CALLDATALOAD] offset=0x{:x} → selector=0x{:08x}", offset, selector);
    }
},
...
0x3c => {
    ...
    if dest_offset + size <= global_mem.len() {
        ensure_memory_size(&mut global_mem, dest_offset + size, &mut execution_context.free_memory_pointer)?;
  ...
  0x57 => {
    if evm_stack.len() < 2 {
        return Err(Error::new(
            ErrorKind::Other,
            format!("EVM STACK underflow on JUMPI (need 2, have {})", evm_stack.len())
        ));
    }
    ...
    println!(
        ...
        insn_ptr, condition, dest, evm_stack.len()
    );
    ...
    if condition.is_zero() {
        ...
        if !valid_jumpdests.contains(&dest) {
            println!("❌ [INVALID JUMPI] destination 0x{:04x} n'est PAS un JUMPDEST valide", dest);
            return Err(Error::new(
                ErrorKind::Other,
...
0x59 => {
    ...
    0x5e => {
        ...
        if safe_len > 0 && src_offset + safe_len <= global_mem.len() && dst_offset + safe_len <= global_mem.len() {
            ensure_memory_size(&mut global_mem, dst_offset + safe_len, &mut execution_context.free_memory_pointer)?;
...
// 0xa0..=0xa4 : LOG0 à LOG4 — VERSION GÉNÉRIQUE
    ...
    let data = if offset + size <= global_mem.len() {
        ensure_memory_size(&mut global_mem, offset + size, &mut execution_context.free_memory_pointer)?;
    ...
    return Ok(JsonValue::Object(result));
            ...
            0xfd => { 
                ...
                let size   = evm_stack.pop().unwrap().low_u64() as usize;
                    ...
                    // PATCH : Ignore tous les REVERT du dispatcher Solidity (cascade selectors)
    // Adapte la plage selon la taille de ton dispatcher (ici 0x0211..=0x02ff, élargis si besoin)
    if (0x0211..=0x03ff).contains(&insn_ptr) {
        println!("⏭️ [REVERT IGNORED] Dispatcher revert à 0x{:04x} ignoré (cascade selectors)", insn_ptr);
        insn_ptr += 1; // Avance le PC pour sortir de la boucle
        continue;
    }
                ...
                if offset + revert_data.len() <= global_mem.len() {
                    ensure_memory_size(&mut global_mem, offset + revert_data.len(), &mut execution_context.free_memory_pointer)?;
